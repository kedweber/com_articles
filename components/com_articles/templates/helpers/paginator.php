...
class ComArticlesTemplateHelperPaginator extends KTemplateHelperPaginator
{
    /**
    ...
    {
        $html  = $pages['first'] ? '<li class="first">'.$this->_bootstrap_link($pages['first'], $this->translate('First')).'</li>' : '';
        ...
        $html  .= $pages['next'] ? '<li class="next">'.$this->_bootstrap_link($pages['next'], $this->translate('Next')).'</li>' : '';
    ...
    protected function _bootstrap_link($page, $title)
    {
        ...
        $html = '<a href="'.$url.'"  data-query="'.http_build_query(array('limit' => $page->limit, 'offset' => $page->offset)).'">'.$this->translate($title).'</a>';
